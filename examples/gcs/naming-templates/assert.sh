...
if kc apply -f "$(dirname "$0")/bad-template.yaml" 2>/tmp/gcs-bad-template.err; then
  fail "admission accepted a template referencing missing label 'no.such/label'"
fi
grep -q "no.such/label" /tmp/gcs-bad-template.err \
  || fail "rejection message should reference 'no.such/label': $(cat /tmp/gcs-bad-template.err)"
...
if kc apply -f "$(dirname "$0")/bad-name.yaml" 2>/tmp/gcs-bad-name.err; then
  fail "admission accepted a template resolving to an invalid bucket name"
fi
grep -q "lowercase" /tmp/gcs-bad-name.err \
  || fail "rejection message should explain bucket name rules: $(cat /tmp/gcs-bad-name.err)"
